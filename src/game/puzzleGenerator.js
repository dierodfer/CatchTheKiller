...
export function generatePuzzle(difficultyId = 'facil', seed = randomSeed()) {
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    ...
    // Personajes: N−1 sospechosos + 1 víctima.
    const names = shuffle(rng, NAMES).slice(0, difficulty.numCharacters)
    const characters = {
    ...
    // 2. SOLUCIÓN
    const solution = generateSolution(rng, map, characters)
    ...
    const killer = identifyKiller(solution.placements, characters.suspects, characters.victim)
